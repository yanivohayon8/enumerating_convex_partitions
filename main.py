...
def create_puzzle():
    ...
    if args.sampling_src_file != "":
        ...
        interior_points,convex_hull_points,_ = sampler.sample_image(num_sampled_points,args.sampling_img_path,output_dir=args.puzzles_dst_folder)
        ...
        interior_points,convex_hull_points,_ = sampler.sample_points_on_circle(args.circle_num_ch,args.circle_num_interior,
                                                                               args.circle_radius,output_dir=args.puzzles_dst_folder)
        board = Board(interior_points=interior_points,convex_hull_points=convex_hull_points)
    else:
    ...
    try:
        creator = AllPartitionsCreator(board,args.puzzles_dst_folder,is_save_partitions_figures=args.save_partitions_figures)
        creator.run()
